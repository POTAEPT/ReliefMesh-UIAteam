...
export const DonationDetailView: React.FC<DonationDetailViewProps> = ({ request, onBack }) => {
  ...
  const handleAction = async () => {
    ...
    if (!ethereum) {
      alert("ไม่พบ MetaMask! กรุณาติดตั้ง Extension ก่อน");
      return;
    }
  ...
  return (
    // 🛠️ Main Container: จัด Flex Column เต็มจอ
    <div className={styles.detailView} style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#F3F4F6' }}>
      ...
      {/* 2. Scrollable Area (ส่วนเนื้อหาที่เลื่อนได้) */}
      <div style={{ flex: 1, overflowY: 'auto', position: 'relative' }}>
        ...
        {/* Card Section (วางซ้อน Map นิดนึงให้สวย) */}
        <div style={{ 
          marginTop: '-40px', 
          ...
          position: 'relative', 
          zIndex: 10,
          padding: '24px', // ✅ เพิ่ม Padding ไม่ให้ชิดขอบ
          minHeight: '500px', // เผื่อความสูงไว้
